...
var graphColor = function() {
	...
	var solveIt = function() {
		...
		var colors = ANIMATIONCONTROL.colors;
		
		var excessColor = null;
		...
		for(var i = 0; i < GRAPH.nodes.length; i++) {
			nodesLeft.push({
				id: GRAPH.nodes[i].getId(),
				degree: GRAPH.nodes[i].getDegree(),
			})
		}
			
		changeColor = true;//Need to select the first color
		nodesLeft.sort(function(nodeA, nodeB) {
		...
		while(nodesLeft.length !== 0) {
			...
			if(changeColor === true){
				...
				nodeColors.push({id: nodesLeft[0].id, color: colors[currentColor]});
			...
			//Continuing the same color
				...
				for(var i = 0; i < nodesLeft.length; i++) {
					...
					if(checkAdjNodesColor(nodesLeft[i], nodeColors, colors[currentColor])) {
						giveUp = false;
	...
	var generateRandomColor = function() {
		return "#" + (generateRandomNumber()).toString(16) + (generateRandomNumber()).toString(16) + (generateRandomNumber()).toString(16);
	}
		
	var generateRandomNumber = function() {
		return Math.floor(Math.random() * (255 - 128) + 128);
	}
	
	var checkAdjNodesColor = function(node, nodes, color) {
	...
	var colorNodes = function() {
		...
		for(var i = 0; i < solution.length; i++) {
			obj = scene.getObjectById(solution[i].id);
			obj.material.color.setHex(solution[i].color);
		}
	}
...
var graphParser = function() {
	...
	var generateGraph = function(nodeSet, edgeSet) {
		GRAPHDRAWER.clearGraph();
		
		initGraph(nodeSet, edgeSet);
	}
	
	//Generate intial positons of nodes.
	var initGraph = function(nodeSet, edgeSet) {
		var label;
		var obj;
